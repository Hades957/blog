...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        response.set_cookie('is_login', True)  # 状态
        response.set_cookie('username', user.username, max_age=7 * 24 * 3600)  # 用户信息和过期时间
        return response
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        # 采用系统自带的认证方法进行认证，默认的认证方法是针对于username字段进行用户名的判断
...
# 写博客视图 继承LoginRequiredMixin，只有登录用户才能进行写博客
